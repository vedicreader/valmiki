...
from valmiki.core import base, home, RouteOverrides
...
def tkn_page(req, auth): return base(token_card(live=bool(api_tkn(auth['id']))), auth, title='API token')
def tkn_new(req, auth): return token_card(mk_api_tkn(auth['id']))
def tkn_del(req, auth): return rm_api_tkn(auth['id']) or token_card()
...
def connect(app):
    ...
    app.get(Routes.tkn)(tkn_page)
    app.post(Routes.tkn)(tkn_new)
    app.post(Routes.tkn_rm)(tkn_del)
    RouteOverrides.nav += [('Token', Routes.tkn, None, True)]
