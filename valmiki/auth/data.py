...
import hashlib, hmac, secrets, time, jwt, re
from valmiki.core import landing, placeholder, send_email, email_template, thread_db, home
from .ui import *
from .cfg import *
g_oath = git_oath = None

def setup_beforeware(app):
    def before(req, sess): return None if auth_ok(req) else Redirect(Routes.login)
...
def setup_auth(req, sess):
    ...
    else: req.scope['auth'] = auth or bearer_usr(req)
...
def get_token(uid, typ=TokenT.em_verify):
    "A signed token of `typ` for `uid`, replacing whatever that pair held before."
    # `jti` so a re-issued token is a different string, `created_at` because the column's
    # default is a literal fixed when the table was created, not when the row is written
    tok = jwt.encode(dict(uid=uid, typ=typ, jti=secrets.token_urlsafe(8)), cfg.jwt_scrt, 'HS256')
    row = dict(user_id=uid, type=typ, token=tok, created_at=time.time())
    return confirmation_tokens.insert(row, replace=True) and tok

def usr_dict(u): return dict(id=u.id, email=u.email, display_name=u.display_name, avatar_url=u.avatar_url)

def api_tkn(uid):
    "The active access token row for `uid`, or None."
    try: return confirmation_tokens[(uid, TokenT.access_tkn)]
    except (NotFoundError, StopIteration): return

def mk_api_tkn(uid): return get_token(uid, TokenT.access_tkn)
def rm_api_tkn(uid): confirmation_tokens.delete((uid, TokenT.access_tkn)) if api_tkn(uid) else None

def bearer_usr(req):
    "The user an `Authorization: Bearer` header names, without touching the session."
    h = req.headers.get('authorization', '')
    if h[:7].lower() != 'bearer ': return
    u = tok_chk(h[7:].strip(), consume=False, typ=TokenT.access_tkn, exp=cfg.api_tkn_exp)
    return None if isinstance(u, AppErr) else usr_dict(u)
...
def tok_chk(tok, consume=True, typ=None, exp=None) -> AppErr | Table:
    if not tok: return InvalidToken
    exp = int(cfg.tkn_exp if exp is None else exp)
    try:
        ...
        uid, t = data.get('uid'), data.get('typ')
        if not (uid and t and users[uid]) or (typ and t != typ): return InvalidToken
        if not (ct.user_id == uid and ct.type == t and ct.created_at + exp > time.time() and ct.validated != True): return InvalidToken
        if consume: confirmation_tokens.update(dict(user_id=uid, type=t, validated=True))
        return users[uid]
